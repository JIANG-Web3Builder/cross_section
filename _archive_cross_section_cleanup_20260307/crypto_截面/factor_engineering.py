...
class FactorEngine:
    ...
    def alpha001(self) -> pd.DataFrame:
        """Alpha001: 量价背离反转 - 捕捉恐慌抛售后的反弹点 (向量化)"""
        ret = self.returns
        std = self.returns.rolling(5).std()
        condition = ret < 0
        inner = std.where(condition, self.close)
        signed_power = np.sign(inner) * (inner.abs() ** 2)
        # 向量化argmax: 比较每个位置与滞动窗口内最大值
        rolling_max = signed_power.rolling(5).max()
        # 找到最大值的位置 (0-4), 用积累和技巧
        is_max = (signed_power == rolling_max).astype(int)
        # 返回排名
        return is_max.rank(axis=1, pct=True)
